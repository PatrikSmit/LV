...
#nepoznata funkcija koju treba aproksimirati
def non_func(x):
	y = 1.6345 - 0.6235*np.cos(0.6067*x) - 1.3501*np.sin(0.6067*x) - 1.1622 * np.cos(2*x*0.6067) - 0.9443*np.sin(2*x*0.6067)
	return y

#dodavanje suma na realne podatke
def add_noise(y):
    np.random.seed(14)
...
#Podjela podataka na train/test skupove
#70% train i 30% test
np.random.seed(12)
...
linearModel = lm.LinearRegression() #Definicija modela tj koji se model koristi
linearModel.fit(xtrain,ytrain) #Ucenje tj estimacija parametara

#Ispis naucenog modela
...
MSE_test = mean_squared_error(ytest, ytest_p) #Mjera uspjesnosti, gvori koliko model grijesi na nevidenim podacima
